...
class TestAccounts(unittest.TestCase):
    def test_init(self):
        ...
        b1 = SavingsAccount(1500)
        self.assertEqual(b1.get_amount_held(), 1500)
        self.assertEqual(b1.get_minimum(), 1000)
        self.assertEqual(b1.get_min_ever_held(), 1500)
        self.assertEqual(b1.get_interest_rate(), 0.1)
        self.assertEqual(b1.is_in_good_standing(), True)
    ...
    def test_withdrawals_and_deposits(self):
        ...
        #    *USE DIFFERENT NUMBERS*
        s2 = CheckingAccount(150)
        s2.withdraw(40)
        self.assertEqual(s2.get_amount_held(), 110)
        self.assertEqual(s2.is_in_good_standing(), True)
        for i in range(5):
            s2.deposit(10)
        self.assertEqual(s2.get_amount_held(), 160)
        with self.assertRaises(ValueError):
            s2.deposit(10)
    ...
    def test_account_closing(self):
        ...
        b3 = SavingsAccount(2000)
        b3.withdraw(500)
        b3.deposit(1000)
        self.assertEqual(b3.get_min_ever_held(), 1500)
        self.assertEqual(b3.is_active(), True)
        final_amount_b = b3.close_account()
        self.assertEqual(final_amount_b, 2975)
        self.assertEqual(b3.is_active(), False)
    ...
    def test_equality(self):
        ...
        a1 = CheckingAccount(1500)
        a1.withdraw(500)
        a2 = CheckingAccount(1000)
        self.assertEqual(a1 == a2, True)
        s1 = SavingsAccount(2250)
        self.assertEqual(s1 == s1, False)
        pass;
